...
export const StudyTheaterModal: React.FC<StudyTheaterModalProps> = ({
  ...
  const [notesViewTab, setNotesViewTab] = useState<'timeline' | 'markdown' | 'photonotes' | 'batch' | 'playlist'>('timeline');
  ...
  // Real-time Photo Notes Subscription & Count
  ...
  const currentVideoPhotoNotesCount = useMemo(() => {
    if (!video?.id) return 0;
    return allPhotoNotes.filter((n) => n.videoId === video.id).length;
  }, [allPhotoNotes, video?.id]);

  // Automatically switch to Photo Notes view if requested
  useEffect(() => {
    if (video?.openPhotoNotes) {
      setViewMode('split');
      setNotesViewTab('photonotes');
    }
  }, [video?.id, video?.openPhotoNotes]);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#0a0d0f] flex flex-col w-screen h-screen overflow-hidden font-sans select-none animate-in fade-in duration-150">
      ...
      <header className="bg-[#11161a] text-white px-3 sm:px-5 py-2 flex items-center justify-between border-b border-slate-800/80 shrink-0 z-20 gap-2 overflow-x-auto no-scrollbar">
        ...
        {/* Center-Left: Photo Notes Action Button (Always Visible!) */}
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-xs font-bold font-mono flex items-center gap-1.5 transition cursor-pointer shrink-0 shadow-xs ${
              viewMode === 'split' && notesViewTab === 'photonotes'
            ...
            title="Open handwritten note photos (JPEG/PNG) for this lecture"
          >
            <Camera className="w-3.5 h-3.5 text-amber-400" />
            <span>📸 Photo Notes</span>
            {currentVideoPhotoNotesCount > 0 && (
              <span className="text-[10px] bg-black/40 text-amber-200 px-1.5 py-0.2 rounded font-mono font-bold">
                {currentVideoPhotoNotesCount}
              </span>
            )}
          </button>
          ...
          {onCompleteTopic && (
            <button
              onClick={() => {
                ...
                setJumpToast('🎉 Lecture marked complete!');
      ...
      <main className="flex-1 flex flex-col lg:flex-row overflow-hidden relative min-h-0 bg-black">
        ...
        <div className="flex-1 bg-black flex flex-col justify-center items-center relative w-full h-full min-h-0">
          ...
          {/* Floating Photo Notes Quick Pill on Video Player (Always Visible) */}
          ...
          <aside className="w-full lg:w-[440px] bg-[#11161a] border-t lg:border-t-0 lg:border-l border-slate-800 flex flex-col shrink-0 p-3.5 sm:p-4 gap-3 text-white overflow-y-auto animate-in slide-in-from-right-4 duration-200">
            ...
            <div className="flex items-center justify-between pb-2 border-b border-slate-800">
              <div className="flex items-center gap-1 bg-slate-950 p-1 rounded-xl border border-slate-800 flex-wrap">
                ...
                <button
                  onClick={() => setNotesViewTab('photonotes')}
                  className={`px-2.5 py-1 rounded-lg text-xs font-bold font-mono flex items-center gap-1.5 transition cursor-pointer ${
                    notesViewTab === 'photonotes'
                      ? 'bg-amber-400 text-black shadow-xs font-black'
                      : 'text-amber-400/90 hover:text-amber-300'
                  }`}
                  title="View & upload handwritten note photos (JPEG/PNG)"
                >
                  <Camera className="w-3.5 h-3.5" />
                  <span>Photo Notes</span>
                  {currentVideoPhotoNotesCount > 0 && (
                    <span className="text-[10px] bg-black/30 px-1 rounded font-mono font-bold">
                      {currentVideoPhotoNotesCount}
                    </span>
                  )}
                </button>
            ...
            {/* ================= TAB 5: PHOTO NOTES (HANDWRITTEN / JPEG / DIAGRAMS) ================= */}
            {notesViewTab === 'photonotes' && (
              <div className="flex-1 flex flex-col min-h-[420px] rounded-2xl overflow-hidden border border-slate-800 shadow-inner">
                <PhotoNotesManager
                  videoId={video.id}
